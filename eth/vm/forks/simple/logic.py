...
from . import constants as simple_constants
...
def _account_load_cost(was_cold: bool) -> int:
    if was_cold:
        return simple_constants.COLD_ACCOUNT_ACCESS_COST
    else:
        return simple_constants.WARM_STORAGE_READ_COST
...
def sload_eip2929(computation: ComputationAPI) -> None:
    ...
    if _mark_storage_warm(computation, slot):
        gas_cost = simple_constants.COLD_SLOAD_COST
    else:
        gas_cost = simple_constants.WARM_STORAGE_READ_COST
...
GAS_SCHEDULE_EIP2929 = GAS_SCHEDULE_EIP2200._replace(
    sload_gas=simple_constants.WARM_STORAGE_READ_COST,
    sstore_reset_gas=5000 - simple_constants.COLD_SLOAD_COST,
)
...
def selfdestruct_eip2929(computation: ComputationAPI) -> None:
    ...
    if _mark_address_warm(computation, beneficiary):
        gas_cost = simple_constants.COLD_ACCOUNT_ACCESS_COST
